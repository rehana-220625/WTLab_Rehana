...
if (!isset($_GET['code'])) {
    set_flash('error', 'No authorization code received from GitHub.');
    header('Location: index.php');
    exit;
}
...
$user_result = get_github_user_info($token_result['token']);
...
create_user_session($user, 'GitHub');

set_flash('success', 'Welcome back, ' . safe($user['name']) . '! You are now logged in with GitHub.');
